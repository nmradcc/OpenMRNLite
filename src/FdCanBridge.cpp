...
// Note: User must provide hfdcan handle and Error_Handler function
// These are typically defined in user's main.c

class FdCanBridge {
public:
    FdCanBridge(FDCAN_HandleTypeDef *hfdcan, CanHubFlow *hub, void (*error_handler)())
        : hfdcan_(hfdcan), canHub_(hub), txPending_(false), writePort_(this), errorHandler_(error_handler) {}
...
void setup_can_bridge(openmrn_arduino::OpenMRN *openmrn, FDCAN_HandleTypeDef *hfdcan, void (*error_handler)()) {
    canBridge = new FdCanBridge(hfdcan, openmrn->stack()->can_hub(), error_handler);
    canBridge->init();
}
